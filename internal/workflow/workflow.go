...
func Run(ctx context.Context, cfg Config) error {
	...
	if cfg.IOStreams.In == nil {
		cfg.IOStreams.In = os.Stdin
	}
	if cfg.IOStreams.Out == nil {
		cfg.IOStreams.Out = os.Stdout
	}
	if cfg.IOStreams.ErrOut == nil {
		cfg.IOStreams.ErrOut = os.Stderr
	}
	...
	var mover datamover.DataMover = datamover.RsyncMover{Client: client}
...
func requiredBytesWithMargin(usedBytes int64, marginPercent int) (int64, error) {
	if usedBytes < 0 {
		return 0, fmt.Errorf("used bytes must be non-negative")
	}
	if marginPercent < 0 {
		return 0, fmt.Errorf("safety margin must be non-negative")
	}
